...
public class Unity3dIoCContainer : IIoCContainer, IServiceLocator, IDependencyInjector
{
    private class TypeData
    {
		...
		public List<KeyValuePair<DependencyAttribute, FieldInfo>> Constructor { get; private set; }
	...
	{
		this.RegisterSingleton<IIoCContainer>(this);
		this.RegisterSingleton<IServiceLocator>(this);
		this.RegisterSingleton<IDependencyInjector>(this);
    }
    ...
    public object Resolve(Type type, string key = null)
    {
        ...
        {
            if (typeData.IsSingleton && typeData.Instance != null)
            {
                return typeData.Instance;
            }
    ...
    private object Setup(Type type)
    {
        ...
        if (ctors.Length > 0)
        {
            ...
            object[] instances = new object[pInfo.Length];

            for (int i = 0; i < pInfo.Length; ++i)
            {
                instances[i] = Resolve(pInfo[i].ParameterType);
            }
    ...
    {
        if (failed) throw new IoCContainerException(string.Format(format, args), null);
    }
